...
// console.log(i); 지역변수i는 중괄호 안에서 (자기가 포함된 지역 안에서)만 사용이 가능한 변수이다.
console.log(test_a());
// test_a()함수 안에서 선언된 변수 i는 test_a()함수 안에서만 의미를 가지고 사용할 수 있다. 그래서 test_a()를 호출해도 변수 i의 값을 활용할 수가 없다.
function test_b() {
    let j = 3;
    return j;
}
...
let y = two(10);
console.log(x + y);
// 매개변수로 활용할 수 있는 인자가 없을 때에는 그 어떤 값을 매개변수로 집어넣더라고 결과에는 변화가 없다.
console.log('');
